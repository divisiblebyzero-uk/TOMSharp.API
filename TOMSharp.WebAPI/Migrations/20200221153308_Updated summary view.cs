...
{
    public partial class Updatedsummaryview : Migration
    {
...
select Project, ResourceType, CASE WHEN Client LIKE '%BAU' THEN 'No' ELSE 'Yes' END as Capitalised, FinanceMonth, FinanceYear, sum(te.dur)/1000/60/60 as Hours
  from timeentries te,
       people p,
       sprints s
 where te.[Start] >= s.[StartDate]
   ...
   and te.[User] = p.Name
group by Project, ResourceType, Client, FinanceMonth, FinanceYear
GO
");
        }
